"""Business constants shared across estimation, control, and evaluation.

Only values that affect computed results live here: seeds, model orders,
controller weights, evaluation parameters, and the data-cache paths that the
business logic reads and writes. Report-only constants (figure/caption/table
paths and plotting colours) live in ``notebooks/shared/constants.py`` instead.
"""
