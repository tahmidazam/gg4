...
The standard robustness study estimates the whole control pipeline *once* from a
single seed and then evaluates that fixed pipeline across many seeds.  Any
seed-to-seed spread it shows therefore conflates two effects: the Brain's
irreducible measurement noise, and the *transferability* of estimates (system
matrices, bands, gains, hyperparameters) calibrated on one Brain to another.
...
the **Transfer** condition (seed-0's pipeline evaluated on every seed) versus the
**Matched** condition (each seed's own pipeline evaluated on itself).
...
measured online — no impulse subtraction or cross-seed noise averaging.
"""
...
from ..control.bands import find_bands, run_muscle_trial
from ..control.gains import calibrate_gains_closed_loop, estimate_x1_proj
from ..estimation.estimators import fit_era_em
from .metrics import _sweep_score, compute_metrics
from .sweep import _run_subset


# Metric keys shared by the aggregation helper and the figure builder.
METRIC_KEYS = [
    "final_dist",
    "frac_in_sector",
    "time_to_thresh",
...
def estimate_bands(seed: int, cfg: dict, verbose: bool = True) -> list[float]:
    ...
    Replays the open-loop u0-only frequency sweep (the channel ``find_bands``
    consumes) and returns ``[S+, S-, E+, E-]`` frequencies in cyc/step.
    """
    ...
    def _job(fi: int, freq: float) -> tuple[int, float, float]:
        dsh, del_ = run_muscle_trial(freq, 1.0, 0.0, seed, **trial_kw)
        return fi, dsh, del_
...
def optimise_hyperparameters(
    ...
    ``use_lqi`` is fixed to True (LQI is a ground-truth design decision here).
    Phase A optimises on the middle-ring subset; Phase B validates the top-N
    configs on the full grid and keeps the best.  The returned ``eval_results``
    are the best config evaluated on ``all_targets`` for this seed (Matched).
    """
    import optuna
    from tqdm.auto import tqdm
