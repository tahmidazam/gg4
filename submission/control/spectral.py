...
A self-contained control stack distinct from the cartesian-control stack: it
operates on *centred* inputs (u_c = u_phys − 0.5) and its controllers use the
instantaneous ``controller(y, t, target)`` interface rather than the archive
interface of :mod:`submission.control.lqi`.  Three controllers (LQG, LQI, MPC)
plus a no-input baseline reproduce a prescribed frequency spectrum in the
population output of the Brain simulator.
"""
...
class LQGController:
    ...
    def _solve_lqr(
        self, q_track: float, r_effort: float, lambda_var: float
    ) -> np.ndarray:
        cmean, C = self._m.cmean, self._m.C
        Q_mean = np.outer(cmean, cmean)
        Q_var = C.T @ C
        Q_lqr = q_track * (
            (1.0 - lambda_var) * Q_mean + lambda_var * Q_var
        ) + 1e-6 * np.eye(self._m.n_x)
        R_lqr = r_effort * np.eye(self._m.n_u)
        P = solve_discrete_are(self._m.A, self._m.B, Q_lqr, R_lqr)
        return np.linalg.solve(
            R_lqr + self._m.B.T @ P @ self._m.B, self._m.B.T @ P @ self._m.A
        )
...
class MPCController:
    ...
    def _build_solver(
        self, H: int, q_track: float, r_u: float, r_du: float, lambda_var: float
    ):
        n, m = self._m.n_x, self._m.n_u
        N = self._m.n_y
        ...
        K_track = (
            1.0 - lambda_var
        ) * Ksys_mean.T @ Ksys_mean + lambda_var * Ksys_full.T @ Ksys_full
        ...
        def solve(
            ...
            e0_full = (
                free_full - Ksys_full_offset - np.tile(tgt_c_seq, N)
            )  # (H*N,)  approx
            ...
            g = 2.0 * (
                q_track
                * (
                    (1.0 - lambda_var) * Ksys_mean.T @ e0_mean
    ...
    def __call__(self, y: np.ndarray, t: int, target: SpectralTarget) -> np.ndarray:
        x_hat = self._kf.step(y, self._u_prev)
        tgt_c_seq = np.array(
            [target(t + k) - self._m.base_mean for k in range(self._H)]
        )
...
def random_search(
    ...
    for _ in range(n_trials):
        ...
        res = run_closed_loop(
            brain_factory, model, ctrl, target, T=T_opt, n_burnin=n_burnin
        )
